...
from src.data_poller import DataPoller, Event
...
async def lifespan(app: FastAPI):
    data_poller.subscribe(
        Event.PRINTER_STATUS, WebSocketHandler.get_instance().handle_update
    )
    data_poller.subscribe(
        Event.PRINT_JOB, WebSocketHandler.get_instance().handle_update
    )
